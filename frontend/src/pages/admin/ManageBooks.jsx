          ...
          {filteredBooks.map(Book => (
            <div key={Book.id} className="p-4 rounded-2xl bg-bg-color border border-border-color shadow-sm flex flex-col justify-between hover:shadow-md transition-shadow relative text-left">
              <div className="flex flex-col gap-3">
                <div className="relative w-full h-32 rounded-xl overflow-hidden bg-bg-secondary flex items-center justify-center">
                  {Book.thumbnail_url ? (
                    <img src={`${import.meta.env.VITE_BACKEND_URL || ''}${Book.thumbnail_url}`} alt={Book.title} className="w-full h-full object-cover object-top" />
                  ) : (
            ...
            <form onSubmit={handleSubmit} className="p-6 flex flex-col gap-5 text-left">
              ...
              <div className="flex items-center gap-2 mt-2">
                <input
                  ...
                  className="w-4 h-4 text-primary bg-bg-secondary border-border-color rounded focus:ring-primary focus:ring-2 cursor-pointer"
                />
                <label htmlFor="show_on_home" className="text-sm font-semibold text-text-primary cursor-pointer select-none">
